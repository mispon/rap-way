...
namespace UnityEditor.Localization
{
    ...
    public class LocalizationEditorSettings
    {
        ...
        internal virtual AddressableAssetEntry GetAssetEntry(Object asset)
        {
            ...
            return GetAssetEntry(asset.GetEntityId());
#else
            return GetAssetEntry(asset.GetInstanceID());
#endif
        }

#if UNITY_6000_4_OR_NEWER
        internal virtual AddressableAssetEntry GetAssetEntry(EntityId entityId)
        {
            var settings = GetAddressableAssetSettings(false);
            if (settings == null)
                return null;

            var guid = GetAssetGuid(entityId);
        ...
        {
#if UNITY_6000_4_OR_NEWER
            throw new NotSupportedException("Instance IDs are not supported on Unity 6000.4 or newer. Use EntityId-aware overloads instead.");
#else
            var settings = GetAddressableAssetSettings(false);
            if (settings == null)
                return null;
...
#if UNITY_6000_4_OR_NEWER
        internal string GetAssetGuid(EntityId entityId)
        {
            Debug.Assert(AssetDatabase.TryGetGUIDAndLocalFileIdentifier(entityId, out string guid, out long _), "Failed to extract the asset Guid");
            return guid;
        }
#endif
